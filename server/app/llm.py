...
def system_prompt() -> list[dict]:
    ...
    header = (
        ...
        "Chat doubles as quick capture (often dictated — expect noise, infer intent). When "
        "a message is a task, thought, or note rather than a question, file it: tasks via "
        "add_todo (fitting section; to_today only if it must happen today), thoughts via "
        "append_note to the inbox note unless they clearly extend an existing note, "
        "'done X' via list_todos + move_todo. If genuinely ambiguous, ask instead of "
        "writing.\n\n"
