...
enum ItemMovementEngine {
    ...
    static func moveItemForward(
        ...
        let targetResult = insertAtFirstGap(target: target, entry: entry)
    ...
    static func moveItemBack(
        ...
        if let crumb = breadcrumb {
            var targetLines: [String] = target.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
                ? [] : target.splitLines()
            if let pos = findParentPosition(lines: targetLines, breadcrumb: crumb) {
                targetLines.insert(newLine, at: pos)
                targetResult = ensureTrailingBlankLine(targetLines).joined(separator: "\n")
            } else {
                targetResult = insertAtFirstGap(target: target, entry: newLine)
            }
        } else {
            targetResult = insertAtFirstGap(target: target, entry: newLine)
        }
    ...
    /// Insert an entry at the first available gap in the target.
    /// A "gap" is a blank line, a heading boundary, or the end of the first item group.
    /// Always ensures a trailing blank line.
    static func insertAtFirstGap(target: String, entry: String) -> String {
        var lines: [String] = target.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
            ? [] : target.splitLines()
        ...
        // Walk lines: find the first blank line or heading after we've seen
        // at least one item → places entry at end of first section.
        var sawItem = false
        var insertPos: Int?
        for (i, line) in lines.enumerated() {
            ...
            if trimmed.hasPrefix("- ") {
                sawItem = true
            } else if sawItem && (trimmed.isEmpty || trimmed.hasPrefix("#")) {
                insertPos = i
                break
            }
        }

        let pos = insertPos ?? lines.count
        lines.insert(entry, at: pos)

        return ensureTrailingBlankLine(lines).joined(separator: "\n")
    }

    /// Ensure lines end with exactly one blank line.
    static func ensureTrailingBlankLine(_ lines: [String]) -> [String] {
        var result = lines
